...
def save_fig(pt_id, fig_id, tight_layout=True, fig_extension="png", resolution=300):
    IMAGES_PATH = Path() / "results-GPU-minutes" / (pt_id + "-all-files")
...
def read_data(pt_id):
    ...
    return pt_pain, pt_hr, pt_steps, pt_calories, pt_sleep, pt_wear, pt_sync, pt_clinic
...
def process_data(pt_id, washout_period, activity_bin_before_pain_survey, activity_bin_after_pain_survey, sleep_binning_direction):
    pt_pain, pt_hr, pt_steps, pt_calories, pt_sleep, pt_wear, pt_sync, pt_clinic = read_data(pt_id)
    ...
    results = results.dropna(subset=['hr', 'steps', 'calories'])
...
def explore_processed_data(pt_id, washout_period, activity_bin_before_pain_survey, activity_bin_after_pain_survey, sleep_binning_direction, gen_imgs):
    ...
    if gen_imgs:
        ...
        plotcorr(results_filtered, gen_imgs)
...
def clustering(pt_id, washout_period, activity_bin_before_pain_survey, activity_bin_after_pain_survey, sleep_binning_direction, n_jobs, gen_imgs, hyperparam_tune=True, optimal_params=None):
    results, results_filtered, pt_pain_filtered_original, adjustment = explore_processed_data(pt_id, washout_period, activity_bin_before_pain_survey, activity_bin_after_pain_survey, sleep_binning_direction, gen_imgs)

    results_filtered_clustered = results_filtered.copy()

    results_filtered_clustered = results_filtered_clustered[["MPQtotal", "hr", "MinutesAsleep"]]
    ...
    return results, results_filtered, results_filtered_clustered, pt_pain_filtered_original, adjustment, best_params, vi

def visualize_clusters(pt_id, washout_period, activity_bin_before_pain_survey, activity_bin_after_pain_survey, sleep_binning_direction, n_jobs, gen_imgs=True, hyperparam_tune=True, optimal_params=None):
    results, results_filtered, results_filtered_clustered, pt_pain_filtered_original, adjustment, best_params, vi = clustering(pt_id, washout_period, activity_bin_before_pain_survey, activity_bin_after_pain_survey, sleep_binning_direction, n_jobs, gen_imgs, hyperparam_tune, optimal_params)
    ...
    if gen_imgs:
        for cluster_index, count in results_filtered_raw_clustered["cluster"].value_counts().items():
            ...
            if cluster_index != -1:
                ...
                plotcorr(cluster_data.loc[:, cluster_data.columns != "cluster"], gen_imgs)
                plt.tight_layout()
    ...
    if not gen_imgs:
        EXCEL_PATH = Path() / "results-GPU-minutes" / (pt_id + "-all-files")
        EXCEL_PATH.mkdir(parents=True, exist_ok=True)

    results.to_excel(Path() / "results-GPU-minutes" / (pt_id + "-all-files") / (pt_id + "_results.xlsx"), index=False)

    return best_params, vi
